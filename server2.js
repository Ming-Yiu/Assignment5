...
app.post('/fileupload', function(req, res){
  ...
  form.parse(req, function(err, fields, files){
    ...
    fs.rename(oldPath, newPath, function(err){
      ...
      faceDetection(newPath, function(data){
        ...
        if(data.NumFaces>1)
        {
          ...
          word+=data.Gender[0];
          word+=". The average age of each of these face is ";
          word+=data.AverageAge[0];
          word+=".";
        }
        console.log(word);
        textToSpeech(word,function(data){
          var oldPath = __dirname+'/TextToSpeechOutput.wav';
    var newPath = __dirname +'/public/TextToSpeechOutput.wav';
    fs.rename(oldPath, newPath, function(err){
      if (err) throw err;
    });
    ...
    //app.use(__dirname,function(req,res,next){ 
    //});
    res.send('File upload complete<br><audio controls><source src="TextToSpeechOutput.wav " type="audio/wav"></audio>');
      })
